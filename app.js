...
if (sessionStorage.getItem('usernameAutosave')) {
	usernameInput.value = sessionStorage.getItem('usernameAutosave');
	usernameSpan.textContent = sessionStorage.getItem('usernameAutosave');
} else {
...
usernameInput.addEventListener('input', () => {
	...
	sessionStorage.setItem('usernameAutosave', usernameInput.value);
});
...
playerMoves.forEach((move) =>
	move.addEventListener('click', () => {
		if (isGameStarted) {
			...
			const selectedMove = Object.values(
				playerMoves
			).filter((selection) => selection.classList.contains('selected'));
...
startBtn.addEventListener('click', () => {
	if (!isGameStarted) {
		...
		if (
			...
			parseInt(compScore.textContent, 10)
		) {
